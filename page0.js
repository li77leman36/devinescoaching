console.log('=== SCRIPT LOADED v3 - REDIRECT TO redirect.html ===');
...
signupBtn.addEventListener('click', () => {
  ...
  if (!userType) {
    alert('Please select a user type (PT or Client)');
    return;
  }
  
  // Set pt field based on user type selection
  const pt = userType === 'pt' ? 1 : 0;
  ...
  auth.createUserWithEmailAndPassword(email, password)
    .then((userCredential) => {
      ...
      .then(() => {
        // Send email verification after successful Firestore write
